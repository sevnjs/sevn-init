function SevnJS(){const license="copyrights Prateek Raj Gautam, soon to be released under Apache 2.0";const version=`v0.8.2`;const grab=(parentidstr)=>{var match=parentidstr.match(/^([^\[]*)\[*(\d*)\]*$/mi);var parentid=match[1];if(match[2].length>0)var no=match[2]
var parentElement=null;try{if(parentid instanceof Object==true){parentElement=parentid}
...
var rend=""
if(block.type=="html1"||block.type=="html2"){var rend=block.content}
else if(block.type=="code"){var rend=coderender(block.content)}
...
else if(block.type=="heading2"){var rend=heading2render(block.content)}
lex[j].render1=rend}
var rend1join=""
...
else{var parentElement=document.querySelectorAll(parentid)[0];}
